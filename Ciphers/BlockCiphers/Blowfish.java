...
class Blowfish extends BlockCipher {
    ...
    public void setKey(byte[] key) {
        if (key == null || (key.length < KEY_SIZES[0] || key.length > KEY_SIZES[1]))
            throw new BlockCipherException(BlockCipherException.KEY_LEN_MANY, "from 32 up to 448 (%8 = 0)", "from 4 up to 56");
        byte[] IV_save = BLOWFISH_A == null ? null : BLOWFISH_A.IV;
        BLOWFISH_A = new Blowfish_algorithm(key);
        BLOWFISH_A.IV = IV_save;
        super.algorithm = BLOWFISH_A;
    }
