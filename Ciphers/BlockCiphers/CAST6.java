...
class CAST6 extends BlockCipher {
    ...
    public void setKey(byte[] key) {
        if (key == null || ((key.length != KEY_SIZES[0]
                && key.length != KEY_SIZES[1]
                && key.length != KEY_SIZES[2]
                && key.length != KEY_SIZES[3]
                && key.length != KEY_SIZES[4])))
            throw new BlockCipherException(BlockCipherException.KEY_LEN_MANY, "128/160/192/224/256", "16/20/24/28/32");
        byte[] IV_save = CAST6_A == null ? null : CAST6_A.IV;
        CAST6_A = new CAST6_algorithm(key);
        CAST6_A.IV = IV_save;
        super.algorithm = CAST6_A;
    }
    ...
    private class CAST6_algorithm extends BlockCipher.BlockCipherAlgorithm {
        ...
        private void generateKeys(byte[] key) {
            ...
            int Mr = 17;
            if (key.length < 32) {
                byte[] key_expanded = new byte[32];
                System.arraycopy(key, 0, key_expanded, 0, key.length);
                KEY_COPY = key_expanded;
            } else {
                KEY_COPY = key;
            }
            int[] KEY_32 = BitUtil.ByteArrays.byteArrayToIntArray(KEY_COPY);
            int[][] Tm = new int[8][24];
            int[][] Tr = new int[8][24];
