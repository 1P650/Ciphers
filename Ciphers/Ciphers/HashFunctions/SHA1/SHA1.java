...
public class SHA1 extends SHA {
    ...
    public byte[] process(byte[] input) {
        ...
        for (int i = 0; i < input_prepared.length; i += 64) {
            ...
            for (int j = 16; j < 80; j++) {
                words_32bit[j] = BitUtil.Rotation.rotateL(words_32bit[j - 3] ^ words_32bit[j - 8] ^ words_32bit[j - 14] ^ words_32bit[j - 16], 1);
            }
            int a = h0;
            int b = h1;
            int c = h2;
            ...
            for (int j = 0; j < 80; j++) {
                ...
                int temp = BitUtil.Rotation.rotateL(a, 5) + f + e + k + words_32bit[j];
                e = d;
                d = c;
                c = BitUtil.Rotation.rotateL(b, 30);
                b = a;
                a = temp;
            }
            h0 = h0 + a;
