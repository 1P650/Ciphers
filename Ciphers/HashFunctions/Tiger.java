...
public class Tiger extends HashFunction {
    ...
    public byte[] process(byte[] input) {
        ...
        for (int i = 0; i < input.length; i += 64) {
            byte[] chunck = new byte[64];
            System.arraycopy(input, i, chunck, 0, 64);
            long[] x = BitUtil.ByteArrays.byteArrayToLongArray(chunck);
            long aa = a;
            long bb = b;
            long cc = c;
            pass(a, b, c, 5, x);
        ...
        return BitUtil.ByteArrays.longArrayToByteArray(new long[]{a, b, c});
    ...
    private void round(long a, long b, long c, long x, long mul) {
        c ^= x;
        byte[] c_b = BitUtil.ByteArrays.longToByteArray(c);
        a -= T1[c_b[0] & 0xff] ^ T2[c_b[2]& 0xff] ^ T3[c_b[4]& 0xff] ^ T4[c_b[6]& 0xff];
        b += T4[c_b[1]& 0xff] ^ T3[c_b[3]& 0xff] ^ T2[c_b[5]& 0xff] ^ T1[c_b[7]& 0xff];
        b *= mul;
    }

    private void pass(long a, long b, long c, long mul, long[] x) {
        round(a, b, c, x[0], mul);
        round(b, c, a, x[1], mul);
        round(c, a, b, x[2], mul);
        round(a, b, c, x[3], mul);
    ...
    private void key_schedule(long[] x) {
    ...
    private byte[] prepare_input(byte[] input) {
        int l_orig = input.length;
        int l_f = l_orig;
        l_orig++;
        while (l_orig % 64 !=0) l_orig++;
        byte[] input_prepared = new byte[l_orig];
        input_prepared[l_f] = (byte) 0x80;
        System.arraycopy(input, 0, input_prepared, 0, input.length);
