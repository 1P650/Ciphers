...
public class SHA224 implements HashFunction {
    ...
    private int h0 = 0xC1059ED8;
    private int h1 = 0x367CD507;
    private int h2 = 0x3070DD17;
    private int h3 = 0xF70E5939;
    private int h4 = 0xFFC00B31;
    private int h5 = 0x68581511;
    private int h6 = 0x64F98FA7;
    private int h7 = 0xBEFA4FA4;
    ...
    public byte[] process(byte[] input) {
        ...
        for (int i = 0; i < input_prepared.length; i += 64) {
            ...
            for (int j = 16; j < i_num; j++) {
                s0 = (BitUtil.BitRotation.rotateR(words_32bit[j - 15], 7)) ^ (BitUtil.BitRotation.rotateR(words_32bit[j - 15], 18)) ^ (words_32bit[j - 15] >>> 3);
                s1 = (BitUtil.BitRotation.rotateR(words_32bit[j - 2], 17)) ^ (BitUtil.BitRotation.rotateR(words_32bit[j - 2], 19)) ^ (words_32bit[j - 2] >>> 10);
            ...
            for (int q = 0; q < i_num; q++) {
                ...
                int S0 = (BitUtil.BitRotation.rotateR(a, 2)) ^ (BitUtil.BitRotation.rotateR(a, 13)) ^ (BitUtil.BitRotation.rotateR(a, 22));
                int S1 = (BitUtil.BitRotation.rotateR(e, 6)) ^ (BitUtil.BitRotation.rotateR(e, 11)) ^ (BitUtil.BitRotation.rotateR(e, 25));
